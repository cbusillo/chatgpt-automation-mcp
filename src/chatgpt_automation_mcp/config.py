...
class Config:
    ...
    CHATGPT_EMAIL: Optional[str] = os.getenv("CHATGPT_EMAIL")
    CHATGPT_PASSWORD: Optional[str] = os.getenv("CHATGPT_PASSWORD")
    ...
    SAVE_SCREENSHOTS_ON_ERROR: bool = os.getenv("SAVE_SCREENSHOTS_ON_ERROR", "true").lower() == "true"
    ...
    def validate(cls):
        ...
        if cls.HEADLESS and cls.DEBUG_MODE:
            warnings.append("Running in headless mode with debug enabled - screenshots only way to debug")
